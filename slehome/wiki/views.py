...
from wiki.markdown import Markdown 	#wiki의 markdown 기능을 위해 사용됨 (자체 제작)
...
def view_page(request, page_name='',page_num=''): #wiki page를 보여주는 역할을 하는 method이다.
	...
	if len(page) > 0 : #page가 존재할 경우
		if page_num != '' and page_num != '1':
			try:
				...
				c={"alert":"페이지가 없습니다.",'location':'/sle/wiki/page/FrontPage'}
	...
	md = Markdown.markdown(page_name, page.content)	#page의 content를 markdown처리해준다.
...
def edit_page(request,page_name,section='0'): #page의 내용을 변경하기 위한 method이다.
	...
	if len(page) > 0 :	#page가 존재할 경우
		...
		if section > 0 :			#section을 이용하여 content부분 중 어느 부분인지를 확인한다.
			...
			md = Markdown.markdown(page_name, content)['edit'] #edit를 위해 markdown 함수에서 처리해서 가져온다.
			content = Markdown.ContentEdit(content, md[section]['line'], md[section]['next_line']) #수정할 부분을 가져온다.
	else: 	#page가 없을 경우
		content = ""

	manual = Markdown.EditManual()
	c={"page_name":page_name,"content":content,"section":str(section),"manual_explain":manual[0],"manual_content":manual[1]}
	navbar = Navbar(request)
	navbar.get_current_path()
	navbar.user_setting()
	c.update(navbar.context_dict())
...
def save_page(request,page_name,section='0'): #수정하거나 새로 만든 page를 저장하는 역할을 하는 method
	...
	if len(page) > 0:
		...
		if section == 0 :
			...
			md = Markdown.markdown(page_name, page.content)['edit']	#부분 편집을 할 부분을 가져오는 처리
			cur_content = Markdown.ContentEdit(page.content, md[section]['line'], md[section]['next_line'])
	...
	page = Page(author=request.user,page_name=page_name, content=content, pub_date=datetime.datetime.now(),new_version=True) #새로운 page 객체를 생성해서 저장 history를 위해 페이지가 수정될 때마다 새로운 버전으로서 저장되게 만듬
	page.save()
...
def upload_page(request,page_name=''):	#file upload 기능을 위한 method
	...
	if request.method == 'POST':	#post가 있을 경우
		...
		c={'alert': '올리신 '+file_name+' 파일이 업로드 되었습니다.','location':'/sle/wiki/page/'+page_name}
...
def download_page(request,page_name='',file_num=''): #파일 다운로드
	if page_name == '' or file_num == '': #파일 순서정보나 페이지 이름이 없을 경우 예외 처리
		c={'alert':'잘못된 다운로드 요청입니다.','location':'/sle/wiki/page/'+page_name}
		return render(request,"wiki/alert.html",c)
